...
class ChatThreadProvider(BaseProvider[ChatThread, ChatThreadCreate, ChatThreadUpdate]):
    ...
    def get_multi_by_user(
        ...
        stmt = (
            ...
            .order_by(
                func.coalesce(ChatThread.last_message_at, ChatThread.updated_at, ChatThread.created_at).desc()
            )
            .offset(skip)
            .limit(limit)
        )
    ...
    def remove_by_thread_id(self, db: Session, *, thread_id: str) -> Optional[ChatThread]:
        ...
        db.execute(delete(ConversationMessage).where(ConversationMessage.session_id == thread_id))
        db.execute(delete(LearningContext).where(LearningContext.session_id == thread_id))
        db.execute(delete(AgentTask).where(AgentTask.session_id == thread_id))
        db.delete(thread)
        db.commit()
        return thread
