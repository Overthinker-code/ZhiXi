...
@router.get("/session/list", response_model=List[ChatThread])
def read_learning_sessions(
    *,
    db: Session = Depends(deps.get_db),
    skip: int = 0,
    limit: int = 100,
    ...
    """Learning Session semantic endpoint; legacy /threads remains compatible."""
    rows = chat_thread_provider.get_multi_by_user(
        db, user_id=str(current_user.id), skip=skip, limit=limit
    )
    return [_thread_to_api(row) for row in rows]
