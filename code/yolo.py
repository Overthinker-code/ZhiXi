...
try:
    ...
    print(f"✅ YOLOv8-Pose模型加载成功: {MODEL_PATH}")
...
def extract_all_persons(results, image_width, image_height):
    """从YOLO结果提取所有人的关键点和检测框"""
    if results[0].keypoints is None:
    ...
    for i, kp in enumerate(keypoints):
        # 检查关键点有效性（YOLOv8-pose 返回17个关键点）
        # 只要关键点不全为0就认为是有效检测
        if np.all(kp == 0):
            print(f"Person {i}: all keypoints are zero, skipping")
            continue
        ...
        print(f"Person {i}: bbox=({x_min:.0f}, {y_min:.0f}, {x_max:.0f}, {y_max:.0f})")
    ...
    print(f"Returning {len(persons)} valid persons")
    return persons
...
async def analyze_frame(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 提取所有人
        persons = extract_all_persons(results, w, h)
        ...
        if len(persons) == 0:
            ...
            return {
                ...
                "image_width": w,
                "image_height": h,
                "debug": "YOLOv8-Pose 对背影检测效果较差，建议使用正面照片"
            }
        ...
        for person in persons:
            ...
            # 规则分类
            behavior_id, confidence, reason = classifier.classify_single_frame(keypoints_norm)
                ...
                if frame_count % sample_interval == 0:
                    ...
                    if keypoints is not None:
                        ...
                        if first_frame_with_persons is None:
                            ...
                            # 提取所有人员信息
                            persons_data = extract_all_persons(results, w, h)
                            if persons_data:
