...
- Vue 项目优先评估 `@vue-flow/core`，不引入 React Flow。
- 初始只加载 8-12 个核心节点。
- 点击节点后请求 `GET /graph/nodes/{id}/neighbors?depth=1`，增量展开相关节点。
- 采用分层/径向布局并对节点尺寸做碰撞约束；展开前固定当前节点位置，避免整图跳动。
- 节点只更新 inspector，不弹出多层 modal。
- 资源、证据、练习和学习路径都以真实关系边表示。
...
- 课程图谱仅完成页面可达性和溢出审计，不在本轮做零散视觉修补，保留为独立竞品调研与集中重构阶段。
